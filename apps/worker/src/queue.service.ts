...
export class QueueService implements OnModuleInit {
  ...
  public async getQueue(
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.RABBITMQ_API_URL}/api/queues/%2f/${queue}/get`,
        {
          method: 'POST',
          headers: {
  ...
  public async addToQueue(
    ...
    try {
      ...
      return {
        success: true,
        message: "Message sent to queue 'mailer' for @MessagePattern",
      };
    } catch (error) {
  ...
  async handleFailedJob(
    ...
    error,
    dbId: string,
  ): Promise<{ retryCount: number; maxRetries: number }> {
    const retryCount =
      (originalMessage.properties.headers?.retryCount || 0) + 1;
    const maxRetries = 5;
    ...
    if (retryCount <= maxRetries) {
      // if there are retries left, send the message to the queue again
      await channel.sendToQueue(
        'mailer',
        Buffer.from(JSON.stringify({ pattern: 'mail.send', data: payload })),
        { persistent: true, headers: headers },
      );
      return { retryCount: retryCount, maxRetries: maxRetries };
    } else {
      throw new MailerMaxRetriesError(
