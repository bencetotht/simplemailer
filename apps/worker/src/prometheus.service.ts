...
export class PrometheusService implements OnModuleInit {
  ...
  constructor(
    @InjectMetric('mailer_concurrent_mails')
    private readonly concurrentMails: Counter<string>, // number of emails processed in last 5 minutes
    @InjectMetric('mailer_active_mails')
    private readonly activeMails: Counter<string>, // number of emails currently being processed
    @InjectMetric('mailer_send_rate') private readonly sendRate: Gauge<string>, // number of emails sent per minute
    @InjectMetric('mailer_mails_sent_total')
    private readonly mailsSentTotal: Gauge<string>, // number of emails sent
    @InjectMetric('mailer_health_status')
    private readonly healthStatus: Gauge<string>, // health status of the mailer service (1 = healthy, 0 = unhealthy)
    @InjectMetric('mailer_queue_size')
    private readonly queueSize: Gauge<string>, // number of emails in the queue
    @InjectMetric('mailer_mails_failed')
    private readonly mailsFailed: Gauge<string>, // number of emails failed
    @InjectMetric('mailer_mails_pending')
    private readonly mailsPending: Gauge<string>, // number of emails pending
