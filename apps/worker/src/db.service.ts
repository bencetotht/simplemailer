...
export class DbService {
  ...
  async getCredentials(accountId: string): Promise<Partial<Account>> {
    try {
      ...
      if (!account)
        throw new ValueError(`Account with id ${accountId} not found`);

      return account;
    } catch (error) {
      throw new ValueError(
        `Failed to get credentials for account with id ${accountId}: ${error}`,
      );
    }
  }
  ...
  async getTemplate(templateId: string): Promise<Template> {
    try {
      ...
      if (!template)
        throw new ValueError(`Template with id ${templateId} not found`);

      return template;
    } catch (error) {
      throw new ValueError(
        `Failed to get template for template with id ${templateId}: ${error}`,
      );
    }
  }
