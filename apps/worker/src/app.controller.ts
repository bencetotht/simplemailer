...
export class QueueController {
  ...
  public async execute(@Payload() data: MailJob, @Ctx() context: RmqContext) {
    ...
    if (
      ...
      // if any of the required fields are missing, fail & don't requeue the message
    ...
    const db = isRetry
      ? await this.dbService.updateLogStatus(
          orginalMessage.properties.headers.dbId,
          Status.RETRYING,
        )
    ...
    try {
      ...
      if (error instanceof MailerError) {
        // if the error is a mailer error, retry the mail
        try {
          const { retryCount, maxRetries } =
            await this.queueService.handleFailedJob(
              channel,
              data,
              orginalMessage,
              error,
          ...
          this.logger.warn(
            `Failed to send mail to ${data.recipient} with job id ${db.id}, retrying ${retryCount}/${maxRetries}: ${error}`,
          );
          await this.dbService.updateLogStatus(db.id, Status.RETRYING);
        } catch (error) {
          if (error instanceof MailerMaxRetriesError) {
            // if maximum retries are exhausted, update the status to failed
            this.logger.error(
              `Failed to send mail to ${data.recipient} with job id ${db.id}: ${error.message}`,
            );
